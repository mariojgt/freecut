        ...
        deleteProject: async (id: string, clearLocalFiles?: boolean) => {
          ...
          if (handle) {
            ...
            if (!fsPermissionGranted) {
              ...
              throw new Error(
                'Filesystem permission denied — project was not deleted. Please grant access and try again.',
              )
            }
          }
          ...
          try {
                ...
                if (typeof handleWithRemove.remove === 'function') {
                  ...
                  for await (const entry of handle.values()) {
                    try {
                      ...
                      allRemoved = false
                      logger.error(
                        `Failed to remove entry "${entry.name}" in project ${id}:`,
                        entryError,
                      )
                    }
                  }
            ...
            if (localFilesDeleted || partialLocalDeletion) {
              ...
              const scope = localFilesDeleted
                ? 'All local files deleted'
                : 'Some local files deleted'
